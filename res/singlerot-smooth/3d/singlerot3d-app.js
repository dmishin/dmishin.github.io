...
(function() {
  ...
  WorkerFlyingCurves = (function() {
    ...
    WorkerFlyingCurves.prototype.loadPattern = function(pattern, colors) {
      ...
      return this.worker.postMessage({
        ...
        size: this.boardSize,
        lanczosOrder: this.lanczosOrder,
        interpSteps: this.interpSteps,
        smoothingPeriod: this.smoothingPeriod,
        timeScale: this.timeScale,
        tubeRadius: this.tubeRadius,
        tubeSides: this.tubeSides
      });
    };
    ...
    WorkerFlyingCurves.prototype.loadUriParameters = function(keys) {
      ...
      loadIntParam = (function(_this) {
        return function(fieldName, keyName, isValid) {
          var val;
          if (keyName in keys) {
      ...
      loadIntParam("chunkSize", "chunkSize", function(s) {
        return s >= 10 && s < 100000;
      });
      loadIntParam("skipSteps", "skipSteps", function(s) {
        return s >= 1 && s < 10000;
      });
      loadIntParam("boardSize", "boardSize", function(s) {
        return s > 0 && (s % 2 === 0);
      });
      loadIntParam("lanczosOrder", "lanczosOrder", function(s) {
        return s >= 0 && s < 10;
      });
      loadIntParam("interpSteps", "interpSteps", function(s) {
        return s >= 1 && s < 100;
      });
      loadIntParam("smoothingPeriod", "smoothingPeriod", function(s) {
        return s >= 1 && s < 10000;
      });
      loadIntParam("tubeSides", "tubeSides", function(s) {
        return s >= 2 && s < 10;
      });
      if ("timeScale" in keys) {
        v = parseFloat(keys["timeScale"]);
        if ((v === v) && v > 0) {
          this.timeScale = v;
        }
      }
      if ("tubeRadius" in keys) {
  ...
  loadRandomPattern = function(size) {
    return curves.loadPattern.apply(curves, makeRandomPattern(size));
  };

  makeRandomPattern = function(size) {
    var cells, colors, key, pattern, r, u1, u2, x, xyRange, y;
    cells = {};
    pattern = [];
    colors = [];
  ...
  bindEvents = function() {
    ...
    E("select-pattern").addEventListener("change", function(e) {
      var rle;
